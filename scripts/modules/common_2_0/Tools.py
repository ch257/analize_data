...
class Tools:
	...
	def str2type(self, value, value_type):
		...
		if value_type == 'str':
			...
			self._errors.raise_error('Unknown type ' + value_type)
			return value
	...
	def shape_column_types(self, columns, all_column_types):
		if self._errors.error_occured:
			return None
			
		column_types = {}
		for col in columns:
			if all_column_types.get(col) != None:
				column_types[col] = all_column_types[col]
			else:
				if all_column_types.get('default') != None:
					column_types[col] = all_column_types['default']
				else:
					self._errors.raise_error('No default column type')
	...
	def shape_column_formats(self, columns, all_column_formats):
		...
		for col in columns:
			if all_column_formats.get(col) != None:
				...
				if all_column_formats.get('default') != None:
					column_formats[col] = all_column_formats['default']
				else:
					self._errors.raise_error('No default column format')
	...
	def escape_sequence(self, seq):
		...
		if seq == "'\\t'":
			...
			self._errors.raise_error('Unknown escape sequence ' + seq)
		return seq
